...
class Comet_Walker_Nav_Menu extends Walker {




	// array to default object code
	...
	public function start_lvl( &$output, $depth = 0, $args = null ) {
		...
		$output .= "{$n}{$indent}<ul$class_names".$this->megamenu.">{$n}";
	}
...
add_filter('wp_edit_nav_menu_walker', 'custom_nav_menu_walker');

function custom_nav_menu_walker(){
	return 'new_class_return';
}

add_filter('wp_update_nav_menu_item', 'update_filed_data', 10, 3);

function update_filed_data($menu_id, $menu_db_id, $args){
...
add_filter('wp_setup_nav_menu_item', 'filed_of_menu_data');

function filed_of_menu_data($menu_item){
	$menu_item->megamenu = get_post_meta( $menu_item->ID, 'menu_item_megamenu', true);
 ...
 * Return Class
 */
class new_class_return extends Walker_Nav_Menu{
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		$indent = ( $depth ) ? str_repeat( $t, $depth ) : '';	
		ob_start(); 
				 ...
				 <p class="field-custom description description-wide">
				 	<label for="edit-menu-item-megamenu-<?php echo $data_object_id; ?>">
				 		...
				 		<?php $options = array();
				 			...
				 			$options['mega-menu-columns'] = 'Megamenu';
